...
app.use(session({
  ...
  cookie: { secure: false }, // bật true khi HTTPS
}));
...
app.engine('handlebars', engine({
  ...
  helpers: {
    ...
    generateStars(r) {
      ...
      let stars = '', full = Math.floor(r), half = (r % 1) >= 0.5 ? 1 : 0, empty = 5 - full - half;
    ...
    // Dùng để tạo danh sách các số trang
  ...
  try {
    const [outstandingCourses, mostViewedCourses, newestCourses, topCategories] =
      await Promise.all([
        ...
        categoryModel.findMostEnrolledPastWeek(5), // TODO: đổi sang findMostPurchasedPastWeek nếu đã tạo
      ]);
